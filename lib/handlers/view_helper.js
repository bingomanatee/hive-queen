...
var view_helper_factory = require('./../utils/resources/view_helper_factory');
...
var mn_template = _.template('<%= key %>_view_helper.js');
...
module.exports = function (root) {
	return parser.Handler(function (params) {
		var view_helper_path = path.resolve(root, _model_name(params.key));
		if (_DEBUG) console.log('VIEW HELPER HANDLER found helper %s at key %s: making root %s'
			, JSON.stringify(params.value, true, 2), params.key, view_helper_path);
		var l = params.gate.latch();

		view_helper_factory(function(err, helper){
			helper(view_helper_path,  params, l);
		});
	});
}; // end export function