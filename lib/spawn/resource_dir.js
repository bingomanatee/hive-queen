...
module.exports = function (root_dir, config, cb) {
	console.log('resources_dir --- root dir: %s, config: %s', root_dir.join(':'), util.inspect(config));
	if (!config) {
		return cb();
	}
	...
	mkdirp(resources_path, function(err){
		...
		_.each(config, function(files, type){
			console.log('type: %s, files: %s', type, util.inspect(files));
			var l =  gate.latch();
			var subtype_dir = resources_dir.concat([type]);
			var subtype_path = subtype_dir.join('/');
			console.log('writing into subtype_path: %s', subtype_path);
			mkdirp(path.resolve(subtype_path), function(){
				_.each(files, function(def){
					console.log('.... resource type %s config: %s', type, util.inspect(def));
