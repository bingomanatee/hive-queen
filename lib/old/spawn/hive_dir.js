...
module.exports = function ( hives_root, hive_config, cb, meta) {
	if (_DEBUG)  {
		console.log('spawn_hive_dir: %s config%s', hive_root, util.inspect(hive_config), hive_config);

		console.log('hd meta: %s', util.inspect(meta));
			...
			spawn_config(hive_dir.concat([hive_config.name + '_config.json']), config, function () {
				if (hive_config.actions) {
					_.each(hive_config.actions, function (action_config, name) {
						...
						spawn_action_dir( actions_dir, action_config, gate.latch(), meta);
					})
				}
				gate.await(cb);
			}, meta)
